...
pub enum OrganizationGroupCommand {
    ...
    ///Wait for a --jmespath condition on organization groups
    Wait(OrganizationGroupWaitArgs),
}
...
#[derive(clap::Args, Debug)]
pub struct OrganizationGroupWaitArgs {
    pub uuid: String,
...
pub async fn run(
    ...
    match command {
        ...
        OrganizationGroupCommand::Wait(args) => {
            let path = format!("{}{}{}", "/api/organization-groups/", args.uuid, "/");
            crate::wait::wait_for(
                    base_url,
                    token,
