...
A scriptable, LLM/agent-friendly command-line tool for Waldur MasterMind. Covers:
...
- **Team management** (customers, projects, users, roles, invitations, organization groups,
  permission reviews)
- **Marketplace** browsing offerings, and provisioning/terminating OpenStack
  tenants/instances/volumes through Waldur's order flow

This is a curated subset, not full API coverage: `list`/`get`/`create`/`update`/`delete`
where Waldur exposes a direct REST operation, plus `provision`/`terminate` for the OpenStack
resources whose lifecycle goes through the marketplace order flow instead of a direct REST
create/delete.
...
`src/output.rs`, `src/pagination.rs`, `src/http.rs`, `src/request.rs`, `src/filter.rs`,
`src/query.rs`, and `src/order.rs` are hand-written and permanent.
...
## Provisioning (marketplace orders)

OpenStack tenants, instances, and volumes aren't created/deleted through a direct REST
endpoint — they go through Waldur's asynchronous **marketplace order** flow. Those resources
get `provision` and `terminate` verbs for it (`openstack instance provision`, `openstack
volume terminate`, …).

`provision` submits a marketplace order and, by default, polls it to completion and prints
the resulting resource. The order body takes the same `--request`/`--request-file`/
`--generate-skeleton` shape as `create`, and the skeleton is the order envelope
(`offering`/`project`/`accepting_terms_of_service`) with the resource's **typed attributes**
(image, flavor, ports, …) filled in under `attributes` — derived from the offering's own
schema:

```bash
# discover an offering URL to provision against
waldur-cli marketplace offering list --filter type=OpenStack.Instance

waldur-cli openstack instance provision --generate-skeleton yaml > vm.yaml
# edit vm.yaml: set offering, project, and the attributes you want
waldur-cli openstack instance provision --request-file vm.yaml
```

`--no-wait` submits and returns the order immediately instead of polling; `--timeout N`
bounds the wait (default 600s). A failed order (`erred`/`rejected`/`canceled`) surfaces its
`error_message` as an error. As with `create`, any field left `null` in the template is
omitted — including nested `attributes` fields — so a template with just the required
attributes filled in submits cleanly.

`terminate` takes the resource's **`marketplace_resource_uuid`** (from `get`/`list`, not the
plugin resource's own uuid) and optionally termination attributes:

```bash
waldur-cli openstack instance terminate <marketplace_resource_uuid> --request '{"delete_volumes": true}'
```
